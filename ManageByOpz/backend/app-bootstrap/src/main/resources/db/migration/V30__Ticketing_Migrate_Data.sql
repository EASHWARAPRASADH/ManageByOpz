...
ON DUPLICATE KEY UPDATE assignment_groups.id=assignment_groups.id;
...
UPDATE tickets t
JOIN ticketing_users u ON t.assigned_to = u.uid OR t.assigned_to = u.email
SET t.assigned_to_user_id = u.id;

UPDATE tickets t
JOIN ticketing_users u ON t.created_by = u.uid OR t.created_by = u.email
SET t.created_by_user_id = u.id;

-- 4. Migrate Roles
INSERT INTO ticketing_roles (name)
SELECT DISTINCT role FROM ticketing_users WHERE role IS NOT NULL AND role != ''
ON DUPLICATE KEY UPDATE ticketing_roles.id=ticketing_roles.id;

UPDATE ticketing_users u
JOIN ticketing_roles r ON u.role = r.name
SET u.role_id = r.id;

-- 5. Migrate Departments
INSERT INTO ticketing_departments (name)
SELECT DISTINCT department FROM ticketing_users WHERE department IS NOT NULL AND department != ''
ON DUPLICATE KEY UPDATE ticketing_departments.id=ticketing_departments.id;

UPDATE ticketing_users u
JOIN ticketing_departments d ON u.department = d.name
