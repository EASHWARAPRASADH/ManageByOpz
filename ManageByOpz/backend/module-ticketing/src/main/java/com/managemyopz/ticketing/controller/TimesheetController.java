...
public class TimesheetController {
    ...
    private final com.managemyopz.ticketing.service.EmailService emailService;
    ...
    public ResponseEntity<?> getTimesheets(
        ...
        return ResponseEntity.ok(ApiResponse.success(stringifyIds(rows)));
    }
    ...
    public ResponseEntity<?> getOrCreateTimesheet(@RequestBody Map<String, Object> body) {
        ...
        if (userId == null || weekStart == null) {
            return ResponseEntity.badRequest().body(ApiResponse.error(400, "user_id and week_start are required"));
        }
        ...
        if (!existing.isEmpty()) {
            return ResponseEntity.ok(ApiResponse.success(stringifyId(new HashMap<>(existing.get(0)))));
        }
        ...
        if (created.isEmpty()) {
            return ResponseEntity.status(500).body(ApiResponse.error(500, "Failed to create timesheet"));
        }

        return ResponseEntity.ok(ApiResponse.success(stringifyId(new HashMap<>(created.get(0))), "Timesheet created successfully"));
    }
    ...
    public ResponseEntity<?> updateTimesheet(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        ...
        if (updated.isEmpty()) {
            return ResponseEntity.status(404).body(ApiResponse.error(404, "Timesheet not found"));
        }
        return ResponseEntity.ok(ApiResponse.success(stringifyId(new HashMap<>(updated.get(0))), "Timesheet updated successfully"));
    }
    ...
    public ResponseEntity<?> deleteTimesheet(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponse.success(Map.of("success", true), "Timesheet deleted successfully"));
    }
