...
import com.managemyopz.ticketing.service.*;
...
public class TicketController {
    ...
    private final TicketingUserRepository  userRepo;
    ...
    public ResponseEntity<?> get(@PathVariable Long id) {
        return ticketRepo.findById(id)
            .map(t -> ResponseEntity.ok((Object) ApiResponse.success(serialize(t))))
            .orElse(ResponseEntity.status(404).body(ApiResponse.error(404, "Ticket not found")));
    }
    ...
    public ResponseEntity<?> create(@RequestBody Map<String,Object> body) {
        try {
            ...
            return ResponseEntity.status(201).body(ApiResponse.created(serialize(t), "Ticket created successfully"));
        } catch (Exception e) {
            return ResponseEntity.status(500).body(ApiResponse.error(500, "Failed to create ticket: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Map<String,Object> body) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(serialize(updated), "Ticket updated successfully"));
        } catch (RuntimeException e) {
            if (e.getMessage().contains("RCA")) return ResponseEntity.badRequest().body(ApiResponse.error(400, e.getMessage()));
            if (e.getMessage().contains("not found")) return ResponseEntity.status(404).body(ApiResponse.error(404, e.getMessage()));
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        ...
        return ResponseEntity.ok(ApiResponse.success(Map.of("success", true), "Ticket deleted successfully"));
    }

    @DeleteMapping("/tickets/all")
    public ResponseEntity<?> deleteAll() {
        ticketRepo.deleteAll();
        return ResponseEntity.ok(ApiResponse.success(Map.of("success", true), "All tickets deleted successfully"));
    }
    ...
    public ResponseEntity<?> saveCustomFields(@PathVariable String id, @RequestBody Map<String, Object> body) {
        ...
        return ResponseEntity.ok(ApiResponse.success(Map.of("success", true), "Custom fields saved successfully"));
    }
    ...
    public ResponseEntity<?> activities(@PathVariable Long id,
        ...
        return ResponseEntity.ok(ApiResponse.success(list.stream().map(this::serializeActivity).toList()));
    }
    ...
    public ResponseEntity<?> addActivity(@PathVariable Long id, @RequestBody Map<String,Object> body) {
        if (body.get("message") == null || body.get("message").toString().isBlank())
            return ResponseEntity.badRequest().body(ApiResponse.error(400, "Message is required"));
        try {
            TicketActivity a = ticketService.addActivity(id, body);
            return ResponseEntity.status(201).body(ApiResponse.created(serializeActivity(a), "Activity added successfully"));
        } catch (Exception e) {
            e.printStackTrace();
            return ResponseEntity.status(500).body(ApiResponse.error(500, "Failed to add activity: " + e.getMessage()));
        }
    }
    ...
    private ResponseEntity<?> ok(List<Ticket> tickets) {
        return ResponseEntity.ok(ApiResponse.success(tickets.stream().map(this::serialize).toList()));
    }
}
