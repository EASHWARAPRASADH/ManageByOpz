...
public class CompanyController {
    ...
    public ResponseEntity<?> getCompany(@PathVariable Long id) {
        ...
        try {
            ...
            if (rows.isEmpty()) {
                return ResponseEntity.status(HttpStatus.NOT_FOUND).body(ApiResponse.error(404, "Company not found"));
            }
            return ResponseEntity.ok(ApiResponse.success(formatCompany(rows.get(0))));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> createCompany(@RequestBody Map<String, Object> body) {
        ...
        try {
            ...
            if (name == null || name.trim().isBlank()) {
                return ResponseEntity.badRequest().body(ApiResponse.error(400, "Company Name is required"));
            }
            ...
            if (createdRows.isEmpty()) {
                return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(ApiResponse.error(500, "Failed to retrieve created company"));
            }
            return ResponseEntity.status(HttpStatus.CREATED).body(ApiResponse.created(formatCompany(createdRows.get(0)), "Company created successfully"));
        } catch (org.springframework.dao.DuplicateKeyException ex) {
            return ResponseEntity.badRequest().body(ApiResponse.error(400, "A company with this name already exists"));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> updateCompany(@PathVariable Long id, @RequestBody Map<String, Object> body) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(formatCompany(updatedRows.get(0)), "Company updated successfully"));
        } catch (org.springframework.dao.DuplicateKeyException ex) {
            return ResponseEntity.badRequest().body(ApiResponse.error(400, "A company with this name already exists"));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> deleteCompany(@PathVariable Long id) {
        ...
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(Map.of("success", true), "Company deleted successfully"));
        } catch (Exception e) {
