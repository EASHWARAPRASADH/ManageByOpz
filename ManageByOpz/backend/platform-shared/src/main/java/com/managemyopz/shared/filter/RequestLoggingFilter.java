 ...
 *   - tenantId  : the org context (from TenantContext)
 *   - userId    : the authenticated user (from SecurityContext)
 *   - method    : HTTP method (GET, POST, etc.)
 *   - uri       : request path
 *
 * Also emits a structured INFO log line at the END of each request:
 *   METHOD=POST URI=/api/v1/hr/employees STATUS=201 DURATION=45ms USER=john@acme.com IP=127.0.0.1
 *
 ...
 */
@Slf4j
@Component("sharedRequestLoggingFilter")
...
public class RequestLoggingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        try {
            ...
            // Tenant and user may be set by JwtAuthFilter which runs later;
            // we pick them up lazily in the finally block after the chain runs.
            filterChain.doFilter(request, response);

        } finally {
            // After chain — enrich with tenant + user that were resolved during the request
            ...
            if (status >= 500) {
                log.error("METHOD={} URI={} STATUS={} DURATION={}ms USER={} TENANT={} IP={} REQUEST_ID={}",
                        request.getMethod(), request.getRequestURI(), status, duration,
                        userId, tenantId, request.getRemoteAddr(), requestId);
            } else if (status >= 400) {
                log.warn("METHOD={} URI={} STATUS={} DURATION={}ms USER={} TENANT={} IP={} REQUEST_ID={}",
                        request.getMethod(), request.getRequestURI(), status, duration,
                        userId, tenantId, request.getRemoteAddr(), requestId);
            } else {
                log.info("METHOD={} URI={} STATUS={} DURATION={}ms USER={} TENANT={} IP={} REQUEST_ID={}",
                        request.getMethod(), request.getRequestURI(), status, duration,
                        userId, tenantId, request.getRemoteAddr(), requestId);
            }
