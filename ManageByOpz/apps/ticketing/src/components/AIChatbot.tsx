...
function MessageText({ text }: { text: string }) {
 ...
 if (inCodeBlock) {
 elements.push(
 <pre key={`code-${i}`} className="bg-gray-900 text-gray-100 p-4 rounded-2xl text-xs overflow-x-auto my-3 border border-gray-700 shadow-xl shadow-black/20">
 <code>{codeLines.join('\n')}</code>
 </pre>
 );
 codeLines = [];
 ...
 if (line.startsWith('### ')) {
 elements.push(<h3 key={i} className="font-semibold text-sn-dark dark:text-white text-base mt-4 mb-1">{parseBold(line.slice(4))}</h3>);
 } else if (line.startsWith('## ')) {
 elements.push(<h2 key={i} className="font-semibold text-sn-dark dark:text-white text-lg mt-5 mb-2">{parseBold(line.slice(3))}</h2>);
 } else if (line.startsWith('# ')) {
 elements.push(<h1 key={i} className="font-semibold text-sn-dark dark:text-white text-xl mt-6 mb-3">{parseBold(line.slice(2))}</h1>);
 } else if (line.startsWith('- ') || line.startsWith('* ')) {
 elements.push(
 <div key={i} className="pl-4 flex gap-3 my-1 group">
 <span className="text-sn-green font-semibold select-none group-hover:scale-150 transition-transform">•</span>
