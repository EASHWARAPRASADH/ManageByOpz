...
export function Dashboard() {
  ...
  const totalEmployeesCount = Number(dnaAnalytics?.totalEmployees ?? (employees?.length || 0));
  ...
  const departmentStats = dnaAnalytics?.departmentBreakdown
    ...
    : Object.entries(
        ...
        name,
        count,
        percentage: totalEmployeesCount > 0 ? Math.round((Number(count) / totalEmployeesCount) * 100) : 0
