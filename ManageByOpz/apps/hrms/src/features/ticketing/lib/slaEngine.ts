...
// src/lib/slaEngine.ts
// Consolidated SLA Engine calculations and metrics calculator
...
export function toMillis(value: SafeAny): number {
  if (!value) return NaN;
...
export function parseSlaDelayMeta(value: SafeAny): SlaDelayMeta {
  let parsed = value;
...
export function parseSlaDelayLogs(value: SafeAny): SlaDelayLogEntry[] {
  let parsed = value;
  if (typeof value === "string") {
    try {
      parsed = JSON.parse(value);
...
export function computeActiveSlaUsage(ticket: SafeAny, nowMs = Date.now()): ActiveSlaUsage {
...
export function getEffectiveSlaDelayState(ticket: SafeAny, nowMs = Date.now()) {
