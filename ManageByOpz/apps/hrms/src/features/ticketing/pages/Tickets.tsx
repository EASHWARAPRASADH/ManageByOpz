...
export function Tickets() {
 ...
 const displayGroups = visibleGroups;
   ...
   const selectedGroupMembers = selectedGroupObj
     ? members.filter(m => m.groupId === selectedGroupObj.id && m.status === 'active')
     : [];
   const visibleMembers = selectedGroupObj && Array.isArray(allUsers)
     ? allUsers.filter(u => selectedGroupMembers.some(m => m.userId === u.id || m.userEmail === u.email))
     : agents;
...
"Hardware": 1.05
 ...
 const hasCategoryAccess = ["admin","super_admin","ultra_super_admin"].includes(profile?.role ||"") ||
  ["arun.g@technosprint.net","swedhasris@gmail.com","ulter@technosprint.net","admin@technosprint.net","admin@managemyopz.local","demo-admin@managemyopz.local","demo-super_admin@managemyopz.local","demo-ultra_super_admin@managemyopz.local"].includes(user?.email || profile?.email ||"");
